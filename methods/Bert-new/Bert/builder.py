...
class ModelBuilder:
    ...
    def train(self, loader, data_len):
        ...
        for data in loader:
            ...
            # 打印输出和目标张量的形状
            #print("predicted_output shape:", predicted_output.shape)  # 应该是 [batch_size, num_classes]
            #print("sentiment shape:", sentiment.shape)               # 应该是 [batch_size]
            if sentiment.dim() > 1 and sentiment.size(1) > 1:  # 检查是否为 one-hot
                sentiment = sentiment.argmax(dim=1)
            ...
            _, prediction = torch.max(predicted_output, dim=1)
            true_pred += torch.sum(torch.tensor(prediction == sentiment))
            # print("prediction:", prediction)
            # print("sentiment:", sentiment)
            #true_pred=torch.sum(prediction == sentiment)
